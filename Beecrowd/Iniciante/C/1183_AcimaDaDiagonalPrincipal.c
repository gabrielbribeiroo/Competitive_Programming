...
void CalcularSoma(float m[][12], float *soma) {
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if (j > i) {
                *soma += m[i][j]; // Soma todos os elementos acima da diagonal principal da matriz
            }
        }
    }
}
...
void CalcularMedia(float soma, float *med) {
    ...
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if (j > i) {
                cont++; // Conta a quantidade de elementos acima da diagonal principal
            }
        }
    }
    
    *med = soma / cont; // Calcula a média de todos os valores acima da diagona principal
}
