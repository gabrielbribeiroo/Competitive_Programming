...
void CalcularSoma(double m[][12], double *soma) {
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if ((j>i) && (i+j<TAM-1)) { // Verifica se o elemento está acima das duas diagonais simultaneamente
                *soma += m[i][j]; // Soma todos os elementos acima de ambas as diagonais
            }
        }
    }
}
...
void CalcularMedia(double soma, double *med) {
    ...
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if ((j>i) && (i+j<TAM-1)) {
                cont++; // Conta a quantidade de elementos acima das diagonais
            }
        }
    }
    
    *med = soma / cont; // Calcula a média de todos os valores acima das diagonais
}
