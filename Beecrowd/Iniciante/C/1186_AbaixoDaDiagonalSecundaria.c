...
Em seguida, calcule e mostre a soma ou a média considerando somente aqueles elementos que estão abaixo da diagonal secundária da matriz.
*/
...
void CalcularSoma(float m[][12], float *soma) {
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if (i + j > TAM - 1) { // Verifica se o elemento está abaixo da diagonal secundária
                *soma += m[i][j]; // Soma todos os elementos abaixo da diagonal secundária
            }
        }
    }
}
...
void CalcularMedia(float soma, float *med) {
    ...
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if (i + j > TAM - 1) {
                cont++; // Conta a quantidade de elementos abaixo da diagonal secundária
            }
        }
    }
    
    *med = soma / cont; // Calcula a média de todos os valores abaixo da diagona secundária
}
