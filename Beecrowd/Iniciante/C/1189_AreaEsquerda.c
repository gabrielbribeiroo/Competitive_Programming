...
void CalcularSoma(double m[][12], double *soma) {
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if ((i>j) && (i+j<TAM-1)) { // Verifica se o elemento está abaixo da diagonal principal e acima da secundária duas diagonais simultaneamente
                *soma += m[i][j]; // Soma todos os elementos da área esquerda
            }
        }
    }
}
...
void CalcularMedia(double soma, double *med) {
    ...
    for (int i=0; i<TAM; i++) {
        for (int j=0; j<TAM; j++) {
            if ((i>j) && (i+j>TAM-1)) {
                cont++; // Conta a quantidade de elementos abaixo da diagonal principal e acima da secundária
            }
        }
    }
    
    *med = soma / cont; // Calcula a média de todos os valores da área esquerda
}
